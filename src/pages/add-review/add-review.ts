...
export class AddReviewPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,private viewCtrl: ViewController,public service:DataServicesProvider) {
    this.data1 = {};
    this.data1.user_id = this.service.user_id;
    //this.data1.username = "";
  ...
  review()
  {
    ...
    // let username = "uiu";
    let user_id = this.data1.user_id;
   ...
   if($.trim(rating).length>0 && $.trim(title).length>0  &&
    ...
    let data1 = JSON.stringify({user_id,user_name,rating,title,description});
    this.service.postReview(data1).subscribe(response => {
    console.log(data1);
    info=response.json();
    //this.result1=this.service.name;
    this.result1 = info[0].user_id;
